...
parser.add_argument('--gpu', default=None, type=int,
                    help='GPU id to use.')
parser.add_argument('--save-path', default="/data/yefan0726/checkpoints/large_batch", type=str,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    op_loss, op_acc = 0, 0
    op_train_loss, op_train_acc= 0, 0
    op_lr = 0
    criterion = nn.CrossEntropyLoss().cuda(args.gpu)
    for lr in grid:
        lr_path = os.path.join(args.save_path,  "_optim_%s_%s_%s_%s"%(args.optimizer, args.dataset, args.batch_size, lr) )
        ...
        if dist.get_rank() == 0:
            ...
            chlr.setLevel('INFO')
            fhlr = logging.FileHandler(os.path.join(lr_path, '%s_%s_%s.log'%(args.optimizer, args.dataset, args.batch_size)) )
        ...
        writer = None
        if dist.get_rank() == 0:
            writer = SummaryWriter(log_dir=lr_path   )

        _loss, _acc = np.inf, 0
        _train_loss, _train_acc = np.inf, 0
        if args.distributed:
        ...
        torch.save({
                    ...
                    os.path.join(os.path.join(args.save_path,  "_optim_%s_%s_%s_%s"%(args.optimizer, args.dataset, args.batch_size, lr)  ), 
                    f"model_ep0.ckpt"))
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            _, _, _ = train_one_epoch(logger, args, model, device, train_loader, optimizer, epoch, writer, criterion, scheduler)
            loss_tmp, acc_tmp, acc5_tmp = test_one_epoch(logger, args, model, device, test_loader, writer, epoch, criterion)
            train_loss_avg, train_acc1_avg, train_acc5_avg = test_one_epoch(logger, args, model, device, train_loader, writer, epoch, criterion)
            if dist.get_rank() == 0:
                logger.info(' Testing epoch: {}, Average loss: {:.4f}, Top1-Acc: {:.4f}, Top5-Acc: {:.4f}, \
                            Train: Average loss: {:.4f}, Top1-Acc: {:.4f}, Top5-Acc: {:.4f}'.format(
                epoch, loss_tmp, acc_tmp, acc5_tmp, train_loss_avg , train_acc1_avg , train_acc5_avg))

            if acc_tmp > _acc:
                _acc = acc_tmp
                _loss = loss_tmp
                _train_acc = train_acc1_avg
                _train_loss = train_loss_avg
            ...
            if epoch % args.save_freq == 0:
                torch.save({
                    'epoch': epoch, 
                    'test_acc': acc_tmp,
                    'train_acc': train_acc1_avg,
                    'test_loss': loss_tmp,
                    'train_loss': train_loss_avg,
                    'net': model.state_dict()}, 
                    os.path.join(os.path.join(args.save_path,  "_optim_%s_%s_%s_%s"%(args.optimizer, args.dataset, args.batch_size, lr)  ), 
                    f"model_ep{epoch}.ckpt"))

        if op_acc < _acc:
            op_acc = _acc
            op_loss = _loss
            op_train_acc = _train_acc
            op_train_loss = _train_loss
            
            op_lr = lr
        if dist.get_rank() == 0:
            logger.info("LR:%.6f, test_loss:%.6f, test_acc:%.4f, train_loss:%.6f, train_acc:%.4f" % (lr, _loss, _acc, _train_loss, _train_acc))

    if dist.get_rank() == 0:
        logger.info("Optimal LR:%.6f, test_loss:%.6f, test_acc:%.4f, train_loss:%.6f, train_acc:%.4f" %(op_lr, op_loss, op_acc, op_train_loss, op_train_acc))
